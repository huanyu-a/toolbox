<?php /*a:6:{s:50:"/app/application/index/view/index/textconvert.html";i:1787024461;s:36:"/app/application/index/view/seo.html";i:1787024468;s:39:"/app/application/index/view/header.html";i:1787218864;s:36:"/app/application/index/view/nav.html";i:1786603123;s:39:"/app/application/index/view/footer.html";i:1787218004;s:37:"/app/application/index/view/link.html";i:1787217514;}*/ ?>
<!DOCTYPE html><html lang="zh-CN"><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0, user-scalable=no" /><title><?php echo htmlentities(app('config')->get('web.textconvert.title')); ?>-<?php echo htmlentities(app('config')->get('web.site.name')); ?></title><meta name="applicable-device" content="pc,mobile" /><meta http-equiv="Cache-Control" content="no-transform" /><meta http-equiv="Cache-Control" content="no-siteapp" /><meta name="keywords" content="<?php echo htmlentities(app('config')->get('web.textconvert.keywords')); ?>" /><meta name="description" content="<?php echo htmlentities(app('config')->get('web.textconvert.description')); ?>" /><meta name="renderer" content="webkit" /><meta name="apple-mobile-web-app-capable" content="yes" /><link rel="icon" href="/favicon.ico" mce_href="/favicon.ico" type="image/x-icon" /><link href="/static/style/site.min.css" rel="stylesheet" type="text/css"/>
...
<div class="container"><div class="tool-wrap">
    <div class="tool-card">
        ...
        <p class="tool-desc">汉字拼音、火星文、文字竖排、文字翻转、文字特效、全角半角、英文大小写、人民币大写、命名转换九种文本转换工具合集，全程浏览器本地运算。</p>
        <ul class="t-tabs" id="tcTabs">
            <li><button type="button" class="t-tab active" data-panel="tcPy">汉字拼音</button></li>
            ...
            <li><button type="button" class="t-tab" data-panel="tcName">命名转换</button></li>
        </ul>

        <!-- 汉字拼音 -->
        <div id="tcPy" class="t-panel tc-panel active">
        ...
        <!-- 命名转换 -->
        <div id="tcName" class="t-panel tc-panel">
            <label class="t-label" for="ccInput">原文（驼峰 / 下划线命名）</label>
            <textarea class="t-area" id="ccInput" rows="8" placeholder="输入要转换的内容，例如：order_id 或 orderId"></textarea>
            <div class="tool-actions">
                <button class="t-btn t-btn-ok" type="button" id="ccCamel">下划线转驼峰</button>
                <button class="t-btn" type="button" id="ccUnder">驼峰转下划线</button>
                <button class="t-btn t-btn-ghost" type="button" id="ccDemo">示例</button>
                <button class="t-btn t-btn-ghost" type="button" id="ccClear">清空</button>
            </div>
            <label class="t-label" for="ccResult">转换结果</label>
            <div class="t-result" id="ccResultBox">
                <button class="t-copy" type="button" data-copy="#ccResultText">复制</button>
                <div id="ccResultText"></div>
            </div>
            <div class="t-error" id="ccError"></div>
        </div>
    </div>
</div></div>
...
(function () {
    ...
    /* 命名转换：驼峰 / 下划线互转 */
    var ccInput = document.getElementById('ccInput'), ccResultBox = document.getElementById('ccResultBox'), ccResultText = document.getElementById('ccResultText'), ccError = document.getElementById('ccError');
    function ccUcfirst(s) { return s.length ? s.charAt(0).toUpperCase() + s.slice(1) : s; }
    function ccEscapeRe(ch) { return ch.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); }
    function ccToCamel(str) {
        return ccUcfirst(str).replace(/([^a-zA-Z][a-z])/g, function (m) {
            return m.replace(/_/g, '').toUpperCase();
        });
    }
    function ccToUnder(str) {
        var t = ccUcfirst(str).replace(/([A-Z]).*?([A-Z])/g, function (m) {
            var last = m.charAt(m.length - 1);
            return m.replace(new RegExp(ccEscapeRe(last), 'g'), '_' + last);
        });
        return t.toLowerCase();
    }
    function ccShowErr(m) { ccResultBox.classList.remove('show'); ccError.textContent = m; ccError.classList.add('show'); }
    function ccHideErr() { ccError.classList.remove('show'); }
    function ccShow(v) { ccHideErr(); ccResultText.textContent = v; ccResultBox.classList.add('show'); }
    function ccConvert(fn) {
        ccHideErr();
        var v = ccInput.value;
        if (!v) { ccShowErr('请输入要转换的内容'); return; }
        ccShow(fn(v));
    }
    document.getElementById('ccCamel').addEventListener('click', function () { ccConvert(ccToCamel); });
    document.getElementById('ccUnder').addEventListener('click', function () { ccConvert(ccToUnder); });
    document.getElementById('ccDemo').addEventListener('click', function () {
        ccInput.value = 'user_name order_id create_time';
        ccResultText.textContent = '';
        ccResultBox.classList.remove('show');
        ccHideErr();
        ccInput.focus();
    });
    document.getElementById('ccClear').addEventListener('click', function () {
        ccInput.value = ''; ccResultText.textContent = ''; ccResultBox.classList.remove('show'); ccHideErr(); ccInput.focus();
    });
})();
</script>
