<?php /*a:6:{s:47:"/app/application/index/view/index/currency.html";i:1787036575;s:36:"/app/application/index/view/seo.html";i:1787024468;s:39:"/app/application/index/view/header.html";i:1787218864;s:36:"/app/application/index/view/nav.html";i:1786603123;s:39:"/app/application/index/view/footer.html";i:1787218004;s:37:"/app/application/index/view/link.html";i:1787217514;}*/ ?>
<!DOCTYPE html><html lang="zh-CN"><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0, user-scalable=no" /><title><?php echo htmlentities(app('config')->get('web.currency.title')); ?>-<?php echo htmlentities(app('config')->get('web.site.name')); ?></title><meta name="applicable-device" content="pc,mobile" /><meta http-equiv="Cache-Control" content="no-transform" /><meta http-equiv="Cache-Control" content="no-siteapp" /><meta name="keywords" content="<?php echo htmlentities(app('config')->get('web.currency.keywords')); ?>" /><meta name="description" content="<?php echo htmlentities(app('config')->get('web.currency.description')); ?>" /><meta name="renderer" content="webkit" /><meta name="apple-mobile-web-app-capable" content="yes" /><link rel="icon" href="/favicon.ico" mce_href="/favicon.ico" type="image/x-icon" />
...
<style>
.rate-panel { margin-bottom: 20px; padding: 18px; border: 1px solid var(--border); border-radius: 10px; background: var(--surface); }
.rate-panel .rp-title { font-size: 16px; font-weight: 600; margin: 0 0 12px; color: var(--text-1); }
.rate-form { display: flex; flex-wrap: wrap; gap: 10px; align-items: center; }
.rate-form input[type="number"] { flex: 0 0 160px; min-width: 0; }
.rate-form select { flex: 1 1 150px; min-width: 0; }
.rate-form .rate-arrow { color: var(--text-3); font-weight: 600; }
.rate-result { margin-top: 14px; padding: 12px 14px; border-radius: 8px; background: var(--surface-2); font-size: 15px; color: var(--text-1); display: none; }
.rate-result .rr-val { color: var(--brand); font-weight: 700; font-size: 18px; }
.rate-note { margin-top: 8px; font-size: 12px; color: var(--text-3); }
.rate-board { margin-top: 12px; padding: 10px 14px; border-radius: 8px; background: var(--surface-2); font-size: 14px; color: var(--text-1); line-height: 1.9; }
.rate-board .rb-item { margin-right: 20px; white-space: nowrap; }
.rate-board .rb-val { color: var(--brand); font-weight: 700; font-size: 16px; }
</style>
...
<script async src="https://www.googletagmanager.com/gtag/js?id=G-T510L8HTF9"></script><script>function gtag(){dataLayer.push(arguments)}window.dataLayer=window.dataLayer||[],gtag("js",new Date),gtag("config","G-T510L8HTF9")</script><div class="container"><div class="tool-wrap"><div class="tool-card"><h2 class="tool-title"><span class="t-ico">💱</span>世界各地货币查询</h2><p class="tool-desc">世界各国货币查询与实时汇率换算：提供全球各国及地区货币名称、符号表示与进位制信息查询，支持美元、欧元、日元、英镑、港币、人民币等主流货币的实时汇率在线换算，外贸、旅游、金融从业者快速查阅。</p>
<div class="rate-panel"><h3 class="rp-title">实时汇率换算</h3><div class="rate-form"><input type="number" id="rateAmount" class="form-control" value="100" min="0" step="any" placeholder="金额" /><select id="rateFrom" class="form-control"><option value="USD" selected>美元 USD</option>
...
<option value="MXN">墨西哥比索 MXN</option></select><span class="rate-arrow">→</span><select id="rateTo" class="form-control"><option value="CNY">人民币 CNY</option>
...
<option value="ZAR">南非兰特 ZAR</option>
<option value="BRL">巴西雷亚尔 BRL</option>
<option value="MXN">墨西哥比索 MXN</option></select><button type="button" class="btn btn-success" id="rateConvert">换算</button></div><div class="rate-result" id="rateResult"></div><div class="rate-board" id="rateBoard" style="display:none;">当前：<span class="rb-item">1 <b class="rb-cur" id="rbCurA">—</b> ≈ <span class="rb-val" id="rbValA">—</span> <b class="rb-cur" id="rbCurB">—</b></span><span class="rb-item">1 <b class="rb-cur" id="rbCurB2">—</b> ≈ <span class="rb-val" id="rbValB">—</span> <b class="rb-cur" id="rbCurA2">—</b></span></div><div class="rate-note" id="rateNote">汇率数据加载中，请稍候…</div></div>
<form id="form1" class="form-horizontal" onsubmit="Public.TableSearch($('#table_huobi'), $('#keyword').val());return false;"> <div class="form-group"> <label class="col-sm-2 control-label">关键词：</label> <div class="col-sm-10"> <input class="form-control" type="text" id="keyword" name="keyword" placeholder="输入国家/地区、货币名称或代码，如：美元、USD、中国"> </div> </div>
 	<div class="form-group">
		<div class="col-sm-12 text-center"><button type="button" class="btn btn-success" id="goSearch" onclick="Public.TableSearch($('#table_huobi'), $('#keyword').val())">查询货币</button> <button type="button" class="btn btn-default" id="btnSample" onclick="$('#keyword').val('USD');Public.TableSearch($('#table_huobi'),'USD');">示例</button> <input type="button" onclick="$('#keyword').val('');Public.TableSearch($('#table_huobi'), '');" value="清空" class="btn btn-default"></div>
	</div>	
</form>
...
</div></div></div><script src="/static/script/jquery-1.11.3.min.js" type="text/javascript"></script><script src="/static/script/bootstrap.min.js" type="text/javascript"></script><script type="text/javascript">var Public={};Public.TableSearch=function(table,keyword){if(keyword){keyword=keyword.replace(/\s+/g,'')}if(!keyword){$('tr',table).each(function(){$(this).show()});return}var pattern=new RegExp(keyword),is_show=false,is_td=false,str='';$('tr',table).each(function(){is_show=false,is_td=false;$('td',this).each(function(){is_td=true;str=$(this).text().replace(/\s+/g,'');if(pattern.test(str)){is_show=true}});if(is_td){if(is_show){$(this).show()}else{$(this).hide()}}})}</script>
<script type="text/javascript">
/* 实时汇率换算（服务端代理 /api/rate.php：60 秒级实时，5 分钟缓存） */
(function(){
  var loading = false;
  var $amount = $('#rateAmount'), $from = $('#rateFrom'), $to = $('#rateTo'),
      $result = $('#rateResult'), $note = $('#rateNote');

  function fetchRate(f, t, amt) {
    return $.ajax({
      url: '/api/rate.php',
      data: { from: f, to: t, amount: amt || 1 },
      dataType: 'json',
      timeout: 12000
    });
  }
  ...
  function fmtTime(s) {
    ...
    var dd = t.getFullYear() + '-' + ('0'+(t.getMonth()+1)).slice(-2) + '-' + ('0'+t.getDate()).slice(-2);
    return dd + ' ' + hh + ':' + mm;
  }
  ...
  /* 用代理返回的单条汇率刷新「当前汇率」展示 */
  function updateBoardWith(d) {
    var fn = $from.find('option:selected').text().replace(/\s*[A-Z]{3}\s*$/, '').trim();
    var tn = $to.find('option:selected').text().replace(/\s*[A-Z]{3}\s*$/, '').trim();
    $('#rbCurA').text(fn); $('#rbCurB').text(tn);
    $('#rbValA').text(fmtRate(d.rate));
    $('#rbCurB2').text(tn); $('#rbCurA2').text(fn);
    $('#rbValB').text(fmtRate(1 / d.rate));
    $('#rateBoard').show();
  }

  function updateBoard() {
    var f = $from.val(), t = $to.val();
    if (f === t) { $('#rateBoard').hide(); return; }
    fetchRate(f, t, 1).done(function (d) {
      if (d && d.success) updateBoardWith(d);
    });
  }

  function loadRates() {
    if (loading) return; loading = true;
    $note.text('汇率数据加载中，请稍候…');
    fetchRate($from.val(), $to.val(), 1)
      .done(function (d) {
        if (d && d.success) {
          updateBoardWith(d);
          $note.text('实时汇率：1 ' + d.from + ' ≈ ' + fmtRate(d.rate) + ' ' + d.to +
            '（' + fmtTime(d.updatedAt) + '，来源 ' + d.source + '），仅供参考。');
        } else {
          $note.text('汇率数据加载失败，请稍后刷新重试。');
        }
      })
      .fail(function () { $note.text('汇率数据加载失败，请检查网络后刷新重试。'); })
      .always(function () { loading = false; });
  }

  function convert() {
    var amt = parseFloat($amount.val());
    if (isNaN(amt) || amt < 0) { $result.hide(); $note.text('请输入有效的金额。'); $amount.focus(); return; }
    var f = $from.val(), t = $to.val();
    $note.text('汇率换算中，请稍候…');
    fetchRate(f, t, amt)
      .done(function (d) {
        if (d && d.success) {
          var disp = d.result >= 100 ? d.result.toFixed(2) : (d.result >= 1 ? d.result.toFixed(4) : d.result.toFixed(6));
          $result.html('<span class="rr-val">' + amt + '</span> ' + f + ' = <span class="rr-val">' + disp + '</span> ' + t).show();
          $note.text('实时汇率：1 ' + f + ' ≈ ' + fmtRate(d.rate) + ' ' + t +
            '（' + fmtTime(d.updatedAt) + '，来源 ' + d.source + '），仅供参考。');
          updateBoardWith(d);
        } else {
          $result.hide();
          $note.text('汇率获取失败：' + (d && d.error ? d.error : '未知错误'));
        }
      })
      .fail(function () { $result.hide(); $note.text('汇率获取失败，请检查网络后重试。'); });
  }

  $('#rateConvert').on('click', convert);
  $('#rateAmount').on('keydown', function (e) { if (e.key === 'Enter') { e.preventDefault(); convert(); } });
  $from.on('change', function () { if ($from.val() === $to.val()) { $to.val($from.val() === 'CNY' ? 'USD' : 'CNY'); } updateBoard(); });
  $to.on('change', function () { if ($to.val() === $from.val()) { $from.val($to.val() === 'CNY' ? 'USD' : 'CNY'); } updateBoard(); });

  loadRates();
})();
</script><div class="container foot-history" id="foot-history">
    <div class="row">
