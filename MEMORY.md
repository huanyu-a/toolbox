...
- **技术栈**：ThinkPHP 5.1（PHP ≥ 5.6）+ Bootstrap 3 + jQuery 1.11.3；**无数据库**（配置存文件）；QQWry.dat 纯真 IP 库（约 10MB，运行时依赖，勿删）；验证码 cccyun/think-captcha
...
- **后台**：公开仓库默认 `/portal/`，**生产真实入口 `/mgr-9gdjm7xrtl/`**（服务器容器环境变量 `ADMIN_PATH=mgr-9gdjm7xrtl`，服务器 `/www/docker/tools/config/admin.php` 也有兜底）；账号 `toolbox` / `Toolbox@2026`；登录页 `https://tool.bx9y.com.cn/mgr-9gdjm7xrtl/index/login.html`；后台功能：网站配置（TDK/名称/导航/页脚/友链）、账号、缓存、文件管理、**百度统计**
...
| 08-26 | 当前 | 56 个工具页；本地 sample-data.js 有未提交修改；**已清理会话**：删除 toolbox 项目下其余全部会话（41 个：21 主会话 + 20 子智能体，含 7 个孤儿目录），仅保留当前会话；备份在 `backup/sessions-backup-2026-08-26/`（sessions.db.bak + 48 个 turn 目录 + delete_keys.json） |
...
- **导航分类**：按功能/场景划分为 6 大类（开发编程/文本处理/计算换算/网络运维/站长辅助/生活趣味）+ 后来新增 Agent 分类（README 写 6 大类，实际含 Agent 共 7 组）
